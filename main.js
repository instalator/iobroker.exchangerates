...
/*
setStates - ECB.JPY.Current err = null state = {"type":"state","common":{"name":"Текущий курс","desc":"Текущий курс","type":"number","role":"state","read":true,"write":false},"native":{},"from":"system.adapter.exchangerates.0","user":"system.user.admin","ts":1627960233489,"_id":"exchangerates.0.ECB.JPY.Current","acl":{"object":1636,"state":1636,"owner":"system.user.admin","ownerGroup":"system.group.administrator"}}
 */

function setStates(obj){
    adapter.getObject(obj.name, function (err, state){
        //adapter.log.error('setStates - ' + obj.name + ' err = ' + JSON.stringify(err) + ' state = ' + JSON.stringify(state));
        if (err || !state){
            let type = 'number';
            if (~obj.name.indexOf('Date')) type = 'string';
                ...
                if (adapter.config['0_' + key]){
                    ...
                    setDev({name: src + '.' + key, desc: obj[key].Name, code: obj[key].NumCode}, function (){
                        ...
                        setStates({
                            ...
                            val:  parseFloat(obj[key].Value.toFixed(4))
                        });
                        setStates({
                            name: src + '.' + key + '.Previous',
                            desc: opt.words('Previous', lang),
                            val:  parseFloat(obj[key].Previous.toFixed(4))
                        });
                        setStates({
...
function parseECB(body){
    ...
    xml2js.parseString(body, {
        ...
        arr.forEach(function (item){
            ...
            const val = parseFloat(parseFloat(item.$.rate).toFixed(4));
